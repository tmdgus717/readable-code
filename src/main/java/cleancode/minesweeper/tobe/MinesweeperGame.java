...
public class MinesweeperGame {
    ...
    private static void actOnCell(String cellInput, String userActionInput) {
        ...
        if (doesUserChooseToOpenCell(userActionInput)) {
            if (isLandMineCell(selectedColIndex, selectedRowIndex)) { //지뢰셀이 true면
    ...
    private static void checkIfGameIsOver() { //check는 관용적으로 void 타입
...
//    }
    ...
    private static boolean isAllCellOpened() {
        return Arrays.stream(BOARD)
                ...
                .noneMatch(cell -> CLOSED_CELL_SIGN.equals(cell)); //npe 가능 why? cell이 null일 수 있다 -> 상수를 대상으로 비교하면 안전하다
    }
...
//                break;
            ...
            case 'j':
                return 9;
            default:
                throw new AppException("잘못된 값입니다");
        }
    }
    ...
    private static void initializeGame() {
        ...
        for (int row = 0; row < BOARD_ROW_SIZE; row++) {
            for (int col = 0; col < BOARD_COL_SIZE; col++) {
                if (isLandMineCell(col, row)) { //!isLandMineCell -> isLandMineCell 로 변경!!! -> 부정연산자를 쓸 필요가 없는 상황이였다
